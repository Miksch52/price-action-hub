    ...
    for e in lb:
        ...
        for h, r in rets.items():
            ...
            if edges[h] is not None:
                eimer_edge[e["stufe"]][h].append(edges[h])
        edge = edges[bk]
        einzelfaelle.append({
...
def log_und_evaluate():
    ...
    out = {
        ...
        "hinweis": ("Forward-Test mit festen Fenstern (seit 2026-09-13): Kurs am Signaltag "
                    "(hebel_ampel.stufe gruen/gelb) vs. Schlusskurs genau 21/50/78 "
                    ...
                    "Ergebnis fest). Kein Retro-Modus (siehe Docstring in hebel_backtest.py)."),
